...
- How does the system behave when the administrator tries to remove or disable the last admin or the account owner? The underlying tailnet API rejects the operation; the provider MUST surface that rejection as a clear, actionable diagnostic that explicitly mentions "last admin or account owner" so the tailnet is not left without an administrator. (Enforcement is API-side; the provider does not perform a proactive admin count.)
...
- **FR-009**: The system MUST surface the underlying tailnet's refusal when an administrator attempts to remove or disable the last administrator (membership with admin role) or the account owner. Specifically: the operation is delegated to the Tailscale Control API, which returns an error in this case; the provider MUST translate that error into a clear, actionable diagnostic that explicitly mentions "last admin or account owner" so the operator understands why the action was rejected. (No proactive provider-side admin count is performed; the API is the source of truth.)
