...
- [ ] T073 **In progress.** Tag `v0.1.0` on `main` after T069/T070 pass (both now complete above). Verify release artifacts on GitHub against `quickstart.md` section 5. Checklist:
   1. ~~Run T069 + T070~~ — complete.
   2. Commit docs/tasks changes → push `002-standalone-membership-provider` → fast-forward `main` → `git tag -a v0.1.0 -m "v0.1.0: initial release" && git push origin v0.1.0`.
   3. Re-run the same end-to-end verification used in T072: download `_SHA256SUMS{,_.sig}` from the live release, import the in-repo `KEYS`, `gpg --verify` against the published fingerprint `3F84 0B5A 363E 8126 1D6B  57F0 1583 C036 5BDD 148C`.
   4. Update `quickstart.md §5` example commands to reference `v0.1.0` instead of the rc.
   5. Close this task with the run-link, asset count (expect 14: 11 archives + manifest + SHA256SUMS + sig), and the release URL.
