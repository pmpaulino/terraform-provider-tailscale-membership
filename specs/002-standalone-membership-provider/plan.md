...
2. **Rename** the Go module from `github.com/tailscale/terraform-provider-tailscale` to `github.com/pmpaulino/terraform-provider-tailscale-membership` (FR-012) and rename the membership resource type from the upstream-prototype `tailscale_tailnet_membership` to `tailscale_membership_tailnet_membership` (FR-011). The Terraform local name is `tailscale-membership` (dashed — Terraform's local-name rule rejects underscores; see `spec.md` Q3 amendment) and operators MUST add `provider = tailscale-membership` to every resource block. Source address stays `pmpaulino/tailscale-membership` and is aliased in every `required_providers` example.
...
- The membership resource is registered in the Plugin SDK's `ResourcesMap` under the key `tailscale_membership_tailnet_membership` (FR-011). Note that the *resource-type prefix* `tailscale_membership_` (underscore) and the *Terraform local name* `tailscale-membership` (dash) intentionally differ — they obey different syntactic rules. Operators bridge the two via `required_providers` aliasing plus `provider = tailscale-membership` on every resource block. See `spec.md` Q3 amendment for the full rationale.
